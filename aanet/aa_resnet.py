...
__all__ = ['aa_resnet26', 'aa_resnet38', 'aa_resnet50', 'aa_resnet101', 'aa_resnet152']
...
class AABottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        if self.downsize:
            self.downsample_att = nn.AvgPool2d(kernel_size=3, stride=2, padding=1)
            self.upsample_att = nn.Upsample(scale_factor=2, mode='bilinear')
    ...
    def forward(self, x):
        ...
        if self.downsize:
            out = self.downsample_att(out)
            out = self.conv2(out)
            out = self.upsample_att(out)
        else:
            out = self.conv2(out)
        out = self.bn2(out)
        out = self.relu(out)
...
def aa_resnet26(**kwargs):
    ...
    """
    return AAResNet(Bottleneck, [2, 2, 2, 2], **kwargs)

def aa_resnet38(**kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return AAResNet(Bottleneck, [2, 3, 5, 2], **kwargs)

def aa_resnet50(**kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return AAResNet(Bottleneck, [3, 4, 6, 3], **kwargs)

def aa_resnet101(**kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return AAResNet(Bottleneck, [3, 4, 23, 3], **kwargs)

def aa_resnet152(**kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return AAResNet(Bottleneck, [3, 8, 36, 3], **kwargs)
